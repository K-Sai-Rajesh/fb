...
export default function CustomerProfilePicture() {
    ...
    const [propic, setPropic] = useState(customer === null ? logo : JSON.parse(decodeURIComponent(customer))?.picture_file);
    ...
    async function Submit() {
        try {
            const upload_data = JSON.parse(decodeURIComponent(customer))
            const { payload } = await dispatch(Register(upload_data))
            navigate('/signup/customer/status', { state: payload })
        } catch (e) {
            console.error(e)
        }
    }

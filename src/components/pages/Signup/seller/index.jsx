...
import { sellerFields } from "../../../../helpers/features";
...
export default function Seller() {
    ...
    const handleSubmit = async () => {
        try {
            sessionStorage.setItem('seller', JSON.stringify({ ...JSON.parse(decodeURIComponent(seller)), ...register.values }))
            if (navigator.geolocation) {
                dispatch(loadon(true))
                navigator.geolocation.getCurrentPosition((position) => {
                    console.log(position.coords)
                    dispatch(loadoff(false))
                    navigate("/signup/seller/location", {
                        state: {
                            accuracy
                                :
    ...
    function SetRegister() {
        ...
        sellerFields?.forEach(field => {
            initial[field?.name] = seller === null ? "" : JSON.parse(decodeURIComponent(seller))[field?.name]
